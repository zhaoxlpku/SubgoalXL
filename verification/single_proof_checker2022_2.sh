...
run_java_then_python() {
    ...
    java -cp "/import/ml-sc-scratch2/xueliangz/isabelle_resources_2022/pisa_jars_4096/pisa_copy$((port-8000)).jar" pisa.server.PisaOneStageServer"$port" > "$java_log_file" 2>&1 &
    JAVA_PID=$!
    ...
    python single_proof_checker2022_2.py --input_file "$input_file" --output_file "$output_file" --port "$port" &
    PYTHON_PID=$!
