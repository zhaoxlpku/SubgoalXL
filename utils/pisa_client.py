...
class PisaEnv:
    def __init__(self,
        ...
        # print("self.isa_path: ", self.isa_path)
        # input(">>>>>")
        self.reset()
    ...
    def reset(self):
        ...
        try:
            # print("server_pb2.IsaPath(path=self.isa_path): ", server_pb2.IsaPath(path=self.isa_path))
            print(self.stub.InitialiseIsabelle(server_pb2.IsaPath(path=self.isa_path)).message) #  "tmp/Isabelle2022_0222_1/bin"
            # print("successfully initialize isabelle!!!!")
            print(self.stub.IsabelleWorkingDirectory(server_pb2.IsaPath(path=self.working_directory)).message)
            # print("successfully set isabelle working directory!!!!")
            # print("the starter_string is {}".format(self.starter_string))
            # print("the context of starter_string is {}".format(server_pb2.IsaContext(context=self.starter_string)))
            print(self.stub.IsabelleContext(server_pb2.IsaContext(context=self.starter_string)).message)
            # print("successfully set isabelle context!!!!")
            # input(">>>>>>>>")
    ...
    @func_set_timeout(1800, allowOverride=True)
    # @func_set_timeout(6000, allowOverride=True)
    ...
    def get_premises_and_their_definitions(self, full_name, only_name, proof_body, debug=False):
        ...
        premises_and_their_definitions = [(premise, self.get_fact_defintion(tls_unique_name, premise)) for premise in
                                          premises]
...
if __name__ == '__main__':
    env = initialise_env(
        8179,
        "/home/xueliang/Isabelle2022",
        "/home/xueliang/Isabelle2022/src/HOL/Computational_Algebra/Primes.thy",
        "/home/xueliang/Isabelle2022/src/HOL/Computational_Algebra"
    )
